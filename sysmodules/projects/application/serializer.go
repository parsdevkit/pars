...
func (s Serializer) GetProjectStuctsFromString(data string) ([]applicationproject.ProjectBaseStruct, error) {
	...
	for _, line := range yamlLines {
		...
		if header.Type == structs.StructTypes.Project {
			...
			if projectHeader.Kind == project.StructKinds.Application {
				...
				if err := s.CompleteProjectInformation(&rawProject); err != nil {
					return nil, err
				}
...
func (s Serializer) GetProjectStuctsFromFile(files ...string) ([]applicationproject.ProjectBaseStruct, error) {
	projects := make([]applicationproject.ProjectBaseStruct, 0)
	for _, file := range files {
		...
		projectStructs, err := s.GetProjectStuctsFromString(string(data))
		if err != nil {
			return nil, err
		}
...
func (s Serializer) CompleteProjectInformation(project *applicationproject.ProjectBaseStruct) error {

	logrus.Debugf("filling project (%v) information", project.Name)

	activeWorkspace, err := s.GetWorkspace(*project)
	if err != nil {
		return err
	}
...
func (s Serializer) GetWorkspace(project applicationproject.ProjectBaseStruct) (*workspace.WorkspaceBaseStruct, error) {
...
func (s Serializer) GetGroup(project applicationproject.ProjectBaseStruct) (*group.GroupBaseStruct, error) {
...
func (s Serializer) GetProjectReferences(prj applicationproject.ProjectBaseStruct) ([]applicationproject.ProjectBaseStruct, error) {
...
func (s Serializer) GetProjectReference(prj applicationproject.ProjectBaseStruct, reference applicationproject.ProjectBaseStruct) (*applicationproject.ProjectBaseStruct, error) {
