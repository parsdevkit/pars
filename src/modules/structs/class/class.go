...
func NewClass(key, value string) Class {
	return Class{
		Key:   key,
		Value: value,
	}
...
func NewClass_KeyOnly(key string) Class {
	return Class{
		Key: key,
	}
}
...
func (s *Class) UnmarshalYAML(unmarshal func(interface{}) error) error {
	...
	if err := unmarshal(&value); err == nil {
		...
		if _, ok := err.(*yaml.TypeError); ok {
			var classs map[string]string
			if err := unmarshal(&classs); err == nil {
				if len(classs) == 1 {
					for key, value := range classs {
						s.Key = key
						s.Value = value
						break
					}
