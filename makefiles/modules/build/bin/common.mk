...
build.binary.vendor: $(SOURCE_ROOT_DIR)/pars.go $(SOURCE_ROOT_DIR)/go.mod
	...
	cd $(SOURCE_ROOT_DIR) && GO111MODULE=on GOCACHE=$(LINUX_TMP_ROOT_DIR)/.gocache GOMODCACHE=$(LINUX_TMP_ROOT_DIR)/.gomodcache GOFLAGS=-mod=vendor GOOS=$(APP_OS) GOARCH=$(APP_ARCH) go build -ldflags="-X 'parsdevkit.net/core/utils.version=$(APP_TAG)' $(BINARY_BUILD_STAGE_FLAG) -buildid=$(APPLICATION_NAME)" -o ../$(BIN_ARTIFACTS_DIR)/$(APP) pars.go
