...
binary/debian/postinst:
	...
	echo "" >> $(DEB_POSTINT_FILE_PATH)
	echo "mkdir -p /$(DEB_DATA_DATABASE_DIR)" >> $(DEB_POSTINT_FILE_PATH)
	echo "chmod -R a+rw /$(DEB_DATA_DATABASE_DIR)" >> $(DEB_POSTINT_FILE_PATH)
	echo "" >> $(DEB_POSTINT_FILE_PATH)
...
package.deb.build.binary: binary/debian-files
	...
	@mkdir -p $(DEB_BASE_DIR)/$(DEB_DATA_DATABASE_DIR)
	@mkdir -p $(DEB_BASE_DIR)/$(DEB_CACHE_DIR)
	@mkdir -p $(DEB_BASE_DIR)/$(DEB_LIB_DIR)
	@mkdir -p $(DEB_BASE_DIR)/$(DEB_SHARE_DIR)
	@mkdir -p $(DEB_BASE_DIR)/$(DEB_DOCS_DIR)
	$(MAKE) build.binary.linux TAG=$(APP_TAG) OS=$(OS_LINUX) ARCH=$(ARCH_LINUX_386)
