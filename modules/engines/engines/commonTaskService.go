...
func (s CommonTaskService) CreateTasksFromTask(init bool, data any, taskFiles ...string) error {
...
func (s CommonTaskService) CreateTasksFromFile(init bool, files ...string) error {
	if len(files) > 0 {
...
func (s CommonTaskService) RemoveTasksFromFile(permanent bool, files ...string) error {
	if len(files) > 0 {
...
func (s CommonTaskService) CreateTasks(tasks []commontask.TaskBaseStruct, init bool) error {
	...
	for _, task := range tasksReadyToCreate {
		...
		if _, err := taskService.Generate(task); err != nil {
			return err
		}
...
func (s CommonTaskService) RemoveTasks(tasks []commontask.TaskBaseStruct, permanent bool) error {
