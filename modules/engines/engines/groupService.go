...
func (s GroupService) CreateGroupsFromTemplate(init bool, data any, groupFiles ...string) error {
...
func (s GroupService) CreateGroupsFromFile(init bool, files ...string) error {
	if len(files) > 0 {
...
func (s GroupService) RemoveGroupsFromFile(permanent bool, files ...string) error {
	if len(files) > 0 {
...
func (s GroupService) CreateGroups(groups []groupStruct.GroupBaseStruct, init bool) error {
	...
	for _, group := range groups {
		if ok := groupService.IsExists(group.Name); ok {
			newModelHash, err := utils.CalculateHashFromObject(group)
			if err != nil {
				return err
			}
			structHash := groupService.GetHash(group.Name)
...
func (s GroupService) RemoveGroups(groups []groupStruct.GroupBaseStruct, permanent bool) error {
	...
	for _, group := range groupsReadyToDelete {
		...
		if _, err := groupService.Remove(group.Name, permanent); err != nil {
			return err
		}
