...
func NewFileTemplateEngine(environment string) FileTemplateEngine {
...
func (s FileTemplateEngine) GenerateByResource(model dataresource.ResourceBaseStruct) error {
	workspaceService := NewWorkspaceService(s.environment)

	for _, layer := range model.Specifications.Layers {
		templateService := NewFileTemplateService(s.environment)
...
func (s FileTemplateEngine) GenerateByTemplate(model filetemplate.TemplateBaseStruct) error {
	workspaceService := NewWorkspaceService(s.environment)

	for _, modelLayer := range model.Specifications.Layers {
		resourceService := NewDataResourceService(s.environment)
...
func (s FileTemplateEngine) GenerateContent(workspace workspace.WorkspaceBaseStruct, project applicationproject.ProjectBaseStruct, resource dataresource.ResourceBaseStruct, template filetemplate.TemplateBaseStruct, layer layerPkg.LayerIdentifier) error {
	projectService := NewApplicationProjectService(s.environment)
...
func (s FileTemplateEngine) CheckGeneration(project applicationproject.ProjectBaseStruct, resource dataresource.ResourceBaseStruct, template filetemplate.TemplateBaseStruct, section dataresource.Section, layer dataresource.Layer) (bool, string, string, string, error) {
	var generate = true
